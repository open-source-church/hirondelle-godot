...
def scrape_eventsub_types(use_cache=True):
    ...
    for tr in table.tbody.find_all("tr"):
        ...
        doc1 = soup.find(lambda p: p.name == "h3" and p["id"] == sub["doclink"][1:])
        # sub["doclink"] = "#"+doc1["id"]
...
"""
        # Adds static vars as Sub
        for sub in subs:
            var_name = "_".join([n.upper() for n in sub["type"].split(" ")])
            class_name = "".join([n.capitalize() for n in sub["type"].split(" ")])
            ...
            content += f"class {class_name} extends Sub:\n"
            content += f"\t##Signal emitted on event.\n"
            content += f"\tsignal event(event: {payload_class})\n"
            content += f"\tfunc _init(_twitching: Twitching):\n"
            content += f"\t\tsuper(_twitching, \"{sub["name"]}\", \"{sub["type"]}\", \"{sub["version"]}\", \"{sub["description"]}\", {payload_class}, {condition_class})\n\n"

            # content += f"## {sub["description"]}[br]\n"
            # content += f"## See [{condition_class}] and [{payload_class}].\n"
            # content += f"static var {var_name} := {class_name}.new()\n\n"
        ...
        write_file("SubsClasses.gd", content)
...
"""
# ## Hack to allow static signal. The name if the signal is [member event].
# signal _event(type:String, version: String, event: TBaseType)
# static var singleton = TwitchingSubs.new()
# ## Signal sent with every event
# static var event: Signal = singleton._event

        # Adds init function
        content += "func _init(_twitching: Twitching):\n"
        content += "\ttwitching = _twitching\n\n"
        for sub in subs:
            var_name = "_".join([n.upper() for n in sub["type"].split(" ")])
            class_name = "".join([n.capitalize() for n in sub["type"].split(" ")])
            condition_class = [p for p in sub["request"] if p["name"] == "condition"][0]["class_name"]
            payload_class = [p for p in sub["payload"] if p["type"] == "event"][0]["class_name"]
            content += f"\t{var_name} = TSubs.{class_name}.new(twitching)\n"
        content += "\n"
        ...
        for sub in subs:
            ...
            # content += f"\t##Signal emitted on event.\n"
            # content += f"\tsignal event(event: {payload_class})\n"
            # content += f"\tfunc _init():\n"
            # content += f"\t\tsuper(\"{sub["name"]}\", \"{sub["type"]}\", \"{sub["version"]}\", \"{sub["description"]}\", {payload_class}, {condition_class})\n\n"

            content += f"## {sub["description"]}[br]\n"
            content += f"## See [{condition_class}] and [{payload_class}].\n"
            content += f"var {var_name} : TSubs.{class_name}\n\n"
            # content += f"var {var_name} := TSubs.{class_name}.new()\n\n"
